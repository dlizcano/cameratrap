...
mapview(st_jitter(datos_sf, 0.0005) , zcol="samp_year")
...
Using the coordinates of the `sf` object (datos_sf) and the function `extract` we get the covariates. 
...
```
It seems the difference in mean occupancy probability is not significant.
