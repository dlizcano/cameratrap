...
Pueden ver una revision muy interesante en el articulo: [![Índice de abundancia relativa y tasa de encuentro con trampas cámara](img/RAI_Mandujano.jpg)](https://doi.org/10.47603/mano.v10n1.389)
...
reportTest[[1]] esfuerzo_muestreo. camera trap operation times and image date ranges reportTest[[2]] number of species by station 
...
```{r}
RAI <- wildlife.data |> filter(spanish_common_name=="lapa") |> mutate (RAI_general=round ( (sum(Events) / sum(Effort) ) * 100, 2)) 
datatable(RAI)
```
...
```{r}
RAI <- wildlife.data |> filter(spanish_common_name=="lapa") |> mutate (RAI_camara=round ( Events / Effort * 100, 2)) 
datatable(RAI)
```
